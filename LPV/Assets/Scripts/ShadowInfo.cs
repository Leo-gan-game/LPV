...
public class ShadowInfo : MonoBehaviour
{
    ...
    List<Matrix4x4> world2ShadowMats = new List<Matrix4x4>(1);
    GameObject[] dirLightCameraSplits = new GameObject[1];
    RenderTexture[] depthTextures = new RenderTexture[1];
    ...
    {
        mainCamera_Splits_fcs = new FrustumCorners[1];
        lightCamera_Splits_fcs = new FrustumCorners[1];
        for (int i = 0; i < 1; i++)
        {
    ...
    void OnDrawGizmos()
    {
        ...
        */
        ...
        FrustumCorners[] fcs = new FrustumCorners[1];
        for (int k = 0; k < 1; k++)
        {
